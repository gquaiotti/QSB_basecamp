...
df <- sqlQuery(channel = dbconnection, query = "SELECT project_id, project_event_id, calendar_id, calendar_event_id FROM dbo.delete_holiday_event_f()")

# Fecha a conexao
odbcClose(dbconnection)

# Remove eventos de calendario
df1 <- df %>% filter(!is.na(calendar_id))

for (i in seq(1, nrow(df1))) {
  
  # end point
  url <- paste("https://basecamp.com/", basecamp_userid,"/api/v1/calendars/", df1[i, "calendar_id"] , "/calendar_events/", df1[i, "calendar_event_id"] ,".json", sep = "")
...
# Remove eventos de projeto
df1 <- df %>% filter(!is.na(project_id))

for (i in seq(1, nrow(df1))) {
  
  # end point
  url <- paste("https://basecamp.com/", basecamp_userid,"/api/v1/projects/", df1[i, "project_id"] , "/calendar_events/", df1[i, "project_event_id"] ,".json", sep = "")
